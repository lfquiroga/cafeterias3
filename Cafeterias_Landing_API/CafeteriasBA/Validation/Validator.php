...
namespace CafeteriasBA\Validation;
...
class Validator {
    ...
    public function _max($fieldName, $maxLength) {
        ...
        if (strlen($value) > $maxLength) {
            $this->addError($fieldName, 'El campo ' . $fieldName . " debe tener un maximo de " . $maxLength . " caracteres.");
        }
    ...
    $email = $this->data[$fieldName];

    if (!preg_match("/^[A-z0-9\\._-]+@[A-z0-9][A-z0-9-]*(\\.[A-z0-9_-]+)*\\.([A-z]{2,6})$/", $email)){

    $this->addError($fieldName, 'El campo ' . $fieldName . " no es un email valido .");
